...
function getLatestHotfixVersion(network: Network, baseVersion: Version): number {
  try {
    ...
    for (const line of lines) {
      // Look for versions matching the base version with a hotfix (e.g., 1.0.12.1, 1.0.12.2)
      const versionMatch = line.match(/(\d+)\.(\d+)\.(\d+)\.(\d+)/)
      if (versionMatch) {
        const [, major, minor, patch, hotfix] = versionMatch.map(Number)
        if (major === baseVersion.major &&
          minor === baseVersion.minor &&
          patch === baseVersion.patch) {
          maxHotfix = Math.max(maxHotfix, hotfix)
        }
      }
    }
    ...
    console.warn('Could not determine latest hotfix version, starting from 1')
    return 0
  }
}
...
function deployHotfix(network: Network, hotfixConfig: HotfixConfig) {
  ...
  // Find the latest hotfix version for this base version
  const latestHotfix = getLatestHotfixVersion(network, baseVersion)
  const nextHotfix = latestHotfix + 1

  // Create new version with hotfix number
...
if (!hotfixConfig.deploymentId) {
  ...
  const deploymentId = getDeploymentIdFromVersion(network, hotfixConfig.baseVersion)
  if (deploymentId) {
